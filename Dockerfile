...
# Create the server.js file with proper line continuation
RUN echo 'const http = require("http");' > server.js && \
    ...
    echo '  console.log(`[${timestamp}] Reading index.html from ${indexPath}`);' >> server.js && \
    echo '' >> server.js && \
    echo '  fs.readFile(indexPath, "utf8", (err, content) => {' >> server.js && \
    echo '    if (err) {' >> server.js && \
    echo '      console.error(`Error reading index.html: ${err.message}`);' >> server.js && \
    echo '      res.writeHead(500);' >> server.js && \
    echo '      res.end(`Server Error: Unable to read index.html`);' >> server.js && \
    echo '      return;' >> server.js && \
    echo '    }' >> server.js && \
    echo '' >> server.js && \
    echo '    console.log(`[${timestamp}] Successfully read index.html (${content.length} bytes)`);' >> server.js && \
